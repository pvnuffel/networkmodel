...
for x in range(1,len(sys.argv)):
    ...
    if len(glob.glob(datafile))==0:
        print "Data file %s not found. Exiting" % datafile
        sys.exit() 
    for endline in open(datafile):pass
    parabola_file.write(endline)
    data=np.loadtxt(datafile)             #save point (mean,var) at last timestep 
    pl.plot(data[:,1], data[:,2]*data[:,2],marker='o',color='black',linestyle='-',markersize=3,label= datafile)

parabola_file.close();

endpoints= np.loadtxt("data/endpoints.txt");
pl.plot(endpoints[:,1], endpoints[:,2]*endpoints[:,2]); 


# parabola = np.polyfit(endpoints[:,1], endpoints[:,2]*endpoints[:,2],2)  #degree 2 = parabola
# plotpar = np.poly1d(parabola)
# pl.plot(plotpar,'--')


##pl.quiver(endpoints[:,1], endpoints[:,2]*endpoints[:,2],endpoints[:,1], endpoints[:,2]*endpoints[:,2],angles='xy',scale=1000,color='r')  #tried this for plotting arrows, failed


xmin =0
xmax = 1
ymin = 0
ymax = 0.25 #dependent on number of realizations 
