...
bool Network::small_world(int N, int K, double beta) {
    if ( ring_lattice(N, K) ) {
        ...
        return false; // making ring lattice failed1
    }
    return true;
}
...
bool Network::all_to_all_coupling() {
    int n = size();
     vector<Node*> nodes = get_nodes();
    for (int a = 0; a < n - 1; a++) {
        if (is_stopped() ) { return false; }
...
void Network::initialize(double mean_coupling, double var_coupling, double mean_preference, double var_preference, double mean_initial_state) {              //verschuif naar klasse Opinion_formation?
	    for (int i = 0; i < size(); i++) {
		...
		coupling = mtrand.randNorm(mean_coupling, var_coupling);
	      ...
	      if (mtrand.rand() < mean_initial_state)        //Bernouilli probability defined as 0.5 by default 
...
double Network::get_coarse_state(){
    double average_state =0;
    int n = size();
    for (int i = 0; i < n ; i++) {
      average_state += node_list[i]->get_state()  ;
    }
    return average_state/n;
}
...
Node::Node(Network* network, string name, stateType state, double coupling, double preference, double utility) {
...
double Node::get_neighbors_state() {
    vector<Node*> neighbors= get_neighbors();
    double average_state=0;
    int degree = neighbors.size();
    for (int i = 0; i < degree; i++) 
      average_state += neighbors[i]->get_state();
    return average_state/degree;
} 

void Node::update_utility_function() {
    double neighbors_state= get_neighbors_state();
    double lambda =  get_coupling();
    double q =  get_preference();
    this->utility =(1-lambda)*q+lambda*(2*neighbors_state-1);
} 
