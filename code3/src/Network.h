...
class Network
{
	...
	bool all_to_all_coupling(); //mean-field.
	...
	void lift(double mean_coupling, double var_coupling, double mean_preference, double var_preference, vector<double> U);  
        ...
        double get_coarse_state(); //get average state of the network
...
class Node
{
    ...
    public:
        ...
        inline double get_coupling() {return coupling;}
        inline double get_preference() {return preference;}
	inline double get_utility(){return utility;}
        ...
        inline void set_coupling( double c) { this->coupling = c;}
        inline void set_preference ( double p) { this->preference = p;}
	//	inline void set_utility ( double p) { this->utility = p;} zie update_utility
        ...
        double get_neighbors_state(); //get average state of neighbors
        void update_utility_function();  //get the difference between the two utility functions
	// double probability_state1(double beta); // normalized probability to go to state 1 in the next step. 
    ...
    private:
        Node();
        Node(Network* network, string name, stateType state, double coupling, double preference, double utility);
        ~Node();
        ...
        vector<double> loc;
        stateType state;
        double coupling; //interaction strength
	double preference; // perceived relative quality
        double utility; //difference between two utility functions, depending on state of the neighbours 
